...
- **D1 — геометрия+рендер ✔ ЯДРО** (2026-07-07, на проде): `worldgen/watabou.py` — порт
  ...
  ОСТАТОК D1: профили типов §3 (пока один универсальный), пуассон-хэтч с подрезкой
  (сейчас упрощённый), реквизит-глифы (саркофаг/трон/алтарь), больше петель (createLoop
  скупится: цикломатика обычно 1-2).
