...
- **D1 — геометрия+рендер ✔ ЯДРО** (2026-07-07, на проде): `worldgen/watabou.py` — порт
  ...
  ОСТАТОК D1: профили типов §3 (пока один универсальный), реквизит-глифы
  (саркофаг/трон/алтарь), точные вероятности спавна (в декомпиляции их не было —
  калибровка по виду), больше петель (цикломатика 1-4, медиана 2).
