...
- **D1 — геометрия+рендер** (ядро, самый большой): порт Watabou-аккреции (§2: ось/
  нечётные/зеркальные seed/grow/planner/петли/cleanup/ротонды/вода/колонны), профили
  типов (данные §3), рендер v4 (§6: иерархия линий, тень, пуассон-штриховка, двери,
  реквизит, нумерация по часовой). Галерея /citydebug/dungeons по типам.
  Гарантии+метрики+тесты.
- **D2 — этажи**: стек этажей, XY-лестницы, ≥2 разнотипных связи, вертикальные циклы,
  UI-переключатель ярусов.
