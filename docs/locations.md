...
`worldgen/floorplan.py`: **детерминированная** раскладка зон в клеточную сетку + SVG —
геометрию решает код, не LLM ([принцип 2](README.md), как в citygen). Kind и spot зоны сами
задают позицию: стойка вдоль дальней стены, очаг у стены, «стол в тёмном углу» — в дальний
от входа угол, «под лестницей» — к лестнице; задние комнаты (private/storage/**cell**) —
полосой позади зала; запираемые комнаты постоя — блоком «2-й этаж» 🔒. Мебель (fixed) —
квадратики, утварь (loose) — точки. Рендер — в карточке пула `/citydebug`; тот же план
пойдёт в игровой UI входа в здание.
