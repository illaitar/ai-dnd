...
3. ✔ ЧАСТИЧНО (2026-07-05): позиции по зонам = состояние живой сцены (`engine/zones.py`:
   ...
   Персист зоны игрока ✔ (pc-блоб). ОСТАЛОСЬ: материализация объектов зон в items/live.db
   (holder=zone:…) — предметы обстановки станут крадомыми/лутаемыми, последний слой
   «всё настоящее» для локаций.
