...
- **Код-раскладчик** гарантирует: мебель на выровненной СЕТКЕ слотов (ряды, не россыпь);
  главный проход от входа через зал свят; свободная клетка перед КАЖДОЙ дверью пристройки
  и подходом к лестнице; `reachable()` — финальная BFS-проверка достижимости (тест).
  Спот-зоны раздаются по слотам МАТЧИНГОМ предпочтений: «у окна» = мин. расстояние до
  оконной стены, «в тёмном углу» = дальний угол неоконной стороны, «под лестницей» = к
  лестнице. Задние комнаты (private/storage/**cell**) — полосой позади зала с дверями-
  проёмами; запираемые комнаты постоя — блок «2-й этаж» 🔒.
- Рендер SVG: окна штрихами, дорожка прохода, лестница, мебель (fixed) квадратиками,
  утварь точками. В карточке пула `/citydebug`; тот же план пойдёт в игровой UI входа.
