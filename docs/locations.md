...
- **Рендер, два слоя**: структурный SVG (дебаг, карточка пула) и **бумажный**
  ...
  решётка камеры…), двери с дугами, окна-засечки, номера зон + легенда (группы схлопнуты:
  «стол ×8»). Галерея всех планов: `/citydebug/plans`. Тот же лист пойдёт в игровой UI
  входа в здание (клики по зонам, туман по комнатам, NPC на плане, боевой грид).
...
2. ✔ ЧАСТИЧНО (2026-07-05): позиции по зонам = состояние живой сцены (`engine/zones.py`:
   нужды×afford×приватность-по-нраву×толчея, посты держат работников, гистерезис переездов,
   «перебирается — к очагу» в ленту; кейс 13 покрыт тестами). Слышимость v1: чужая зона —
   «краем уха» обрывок в ленту/память, сплетни только в своей зоне, обращения доходят через
   зал; зоны — в промпте NPC («Ты сейчас — стол у окна»). ОСТАЛОСЬ: материализация объектов
   зон в items/live.db (holder=zone:…) + персист зоны игрока.
3. Разговор-объект + долги + СЦЕНАРНЫЙ БЕНЧ (кейсы 2, 4, 5, 10, 11, 15 — живой LLM).
