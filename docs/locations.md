...
1. ✔ **Зоны в пуле + furnisher + дебаг-браузер** `/citydebug` — обставлено 24/181 key,
   промпт/шаблоны отлажены (досыпать весь пул — механически, `scripts/furnish.py --kind key`).
...
3. ✔ ЧАСТИЧНО (2026-07-05): позиции по зонам = состояние живой сцены (`engine/zones.py`:
   ...
   ОСТАЛОСЬ: материализация объектов зон в items/live.db (holder=zone:…) + персист зоны игрока.
4. Разговор-объект + долги + СЦЕНАРНЫЙ БЕНЧ (кейсы 2, 4, 5, 10, 11, 15 — живой LLM). ← СЛЕДУЮЩИЙ
5. Дирижёр импульсов (1, 3, 12).
6. Салиентность событий (6) → мостик к deed-журналу ([entities.md](entities.md)).
7. Улица-зоны — тем же контуром после интерьеров.
