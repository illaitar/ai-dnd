...
- ✔ (2026-07-06) **Единый `resolve(text)`** — `engine/resolve.py`: реестр PRIMITIVES
  ...
  предметы ЧУЖИХ зон («подойди и возьми» планируется честно). ДАЛЬШЕ по цепочкам:
  reach-автовставка move, deal-гейт (want×stake×бросок×обязательство: контракт giver=PLAYER,
  эскроу, цель в агенду NPC), перцептивные звенья (подслушать=бросок→ярус слуха),
  прерывание цепи салиентным событием.
