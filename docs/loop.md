...
- ✔ (2026-07-06) **Единый `resolve(text)`** — `engine/resolve.py`: реестр PRIMITIVES
  ...
  прочие виды: просрочка = возврат. DC дышит нравом: стражница 22, горожанка 15. ДАЛЬШЕ по
  цепочкам: перцептивные звенья (подслушать=бросок→ярус слуха + чистка DM-снимка),
  прерывание цепи салиентным событием, reach-автовставка move.
