...
PROMPTS = {
    ...
    "narrator": (
        ...
        "выдумывай вариантов (если NPC зовут «Тоблен Стоунхилл» — так и оставляй, не «Каменный Холм»). "
...
_DS_REINFORCE = {
    ...
    "router": (
        ...
        "«осмотреться/оглядеться/что вокруг/куда идти» — это inspect (общий осмотр, БЕЗ броска), НЕ search; "
        "search — ТОЛЬКО активный поиск спрятанного («обыскать», «искать тайник», «пошарить»)."),
}
