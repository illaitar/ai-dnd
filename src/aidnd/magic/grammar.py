...
законописца); сам ЗАКОН круга проявляет LLM (inscribe), строго в пределах бюджета. Без LLM —
детерминированный фоллбэк fallback_law.
...
def fallback_law(comp) -> dict:
    """Детерминированный закон (офлайн/LLM недоступен): скромный, по старой табличной логике."""
    g = load()
    pts = normalize(comp)
    kinds = [(p, g["all"][p["id"]]) for p in pts]
    elems = [(p, e) for p, e in kinds if e["axis"] == "element"]
