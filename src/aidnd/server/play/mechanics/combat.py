...
from aidnd.combat import dungeon, from_npc, from_pc, lair_name, pick_encounter, resolve
...
def _lairs() -> list:
    ...
    if _S.get("lairs") is None:
        ...
        for i in range(PB["lairs"]):
            ...
            cr = PB["lair_cr_near"] + (PB["lair_cr_far"] - PB["lair_cr_near"]) * (
                i / max(1, PB["lairs"] - 1)
            )
            env = rng.choice(envs)
            ...
            out.append(
                {
                    "id": f"lair:{i}",
                    "name": lair_name(units, random.Random(f"ln|{i}")),
                    "env": env,
                    "cr": round(sum(u.cr for u in units), 2),
                    "n": len(units),
                    "foe": (units[0].name if units else "?"),
                    "x": round(490 + math.cos(ang) * rr, 1),
                    "y": round(350 + math.sin(ang) * rr * 0.72, 1),
                }
            )
        _S["lairs"] = out
    return [
        {**l, "cleared": bool(_store().flag_get(_wid(), f"cleared|{l['id']}"))} for l in _S["lairs"]
    ]
...
def _mint_badge(rank_idx: int, holder: str = "pc") -> str:
    ...
    iid = _put_item(
        ...
        "document",
        tier="fine",
        note="удостоверение приключенца",
        holder=holder,
    )
...
def _guild_gate(job_cr: float) -> dict | None:
    ...
    if owner != "pc":  # someone else's badge — the steward checks
        ...
        pr, pw = (
            random.Random(f"lie|{iid}|{_gt()}").randint(1, 20),
        ...
        if me < them:  # rumbled — badge seized, marked
            ...
            return {
                "error": "Распорядитель прищуривается: «Это не твой жетон». Его изымают, "
                "а тебе — чёрную метку.",
                "dice": dice,
            }
...
def _guild_bid() -> str | None:
    ...
    for bid in sorted(set((_S.get("cr2b") or {}).values())):
        info = _binfo(bid)
        sc = (
            (2 if "гильд" in info["kind"].lower() else 0)
...
def _guild_status() -> dict:
    ...
    return {
        "member": True,
        "rank": GUILD_RANKS[rank]["name"],
        "rank_idx": rank,
        "cr_max": GUILD_RANKS[rank]["cr_max"],
        "own": owner == "pc",
        "closed": closed,
        "next": (nxt["name"] if nxt else None),
        "next_need": (nxt["need"] if nxt else None),
        "marked": bool(_store().flag_get(_wid(), "guild_mark|pc")),
    }
...
def _duel_wrapup(enc, cb) -> dict:
    ...
    if st == "won" and p:
        ...
        _npc_save(pid)
        out["narr"].append(
            f"Ты повержен. Очнулся в грязи — кошель легче на {take} зм."
            if take
...
def _combat_wrapup(enc, cb) -> dict:
    ...
    if st == "won":
        ...
        ct = next(
            (
                c
                for c in _store().contracts(_wid(), "active")
                if c.get("kind") == "clear" and c.get("target") == l["id"]
            ),
            None,
        )
        if ct:
        ...
        out["narr"].append(
            "Тьма. Ты очнулся у городских ворот — жив, но растрёпан"
...
def _npc_delves() -> list:
    ...
    if r["status"] == "won":
        ...
        for pid, p in duo:
            ...
            p.state.memory.add(
                f"мы с напарником зачистили {l['name']} по заказу гильдии",
                _mt(),
                0.8,
                about=[d0 for d0, _ in duo],
            )
            _npc_save(pid)
