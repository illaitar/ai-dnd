...
Вызывается на смене фазы суток из тика мира (см. main._world_tick).
"""
...
def _gate_ok(kind: str, p) -> bool:
    ...
    if pk.gate == "rogue":
        return p.role in ("головорез", "бродяга", "наёмник") \
...
def _candidates(p, place_idx: dict, keynode: dict, kps: list, rng) -> list:
    ...
    for kind in ("street", "patrol", "prowl"):             # мобильные места — точка на графе
        if kps and _gate_ok(kind, p):
            out.append(society.Candidate(kind, kps[hash((p.state.config.id, kind, _gt() // 1440)) % len(kps)]))
    return out
