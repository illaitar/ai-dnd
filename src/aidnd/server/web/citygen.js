...
export function drawCity(ctx, W, H, opts={}){
  ...
  // кварталы
  for(const c of city){ if(c===square)continue; const ward=shrink(c.poly,0.86);
    ctx.fillStyle='#cdb585'; pathP(ward); ctx.fill();
    ...
    for(const pl of plots){ if(pl.length<3)continue; const inset=shrink(pl,0.82); if(area(inset)<18)continue;
      if(rng()<0.07){ctx.fillStyle='#9fae6e';pathP(inset);ctx.fill();ctx.strokeStyle='rgba(60,80,40,.4)';ctx.lineWidth=0.7;pathP(inset);ctx.stroke();continue;}
      ctx.save();ctx.translate(1.1,1.4);ctx.fillStyle='rgba(40,28,12,.22)';pathP(inset);ctx.fill();ctx.restore();
      ctx.fillStyle=ROOFS[(rng()*ROOFS.length)|0];pathP(inset);ctx.fill();
      const[a,b]=longestEdge(inset),dir=norm([b[0]-a[0],b[1]-a[1]]),cc=centroid(inset),L=Math.sqrt(area(inset))*0.42;
      ...
      ctx.strokeStyle='rgba(40,28,12,.5)';ctx.lineWidth=0.8;pathP(inset);ctx.stroke();
      hits.push({x:cc[0],y:cc[1],r:Math.max(7,Math.sqrt(area(inset))*0.55),id:`house:${seed}:${Math.round(cc[0])}_${Math.round(cc[1])}`,kind:'home',house:true});
    }
  }
  // площадь
  ...
  // достопримечательности по сторонам света
  const Rl=Math.min(W,H)*0.30, used=new Set([square]);
  for(const b of buildings){ if(b.kind!=='building')continue; const L=Math.hypot(b.dx,b.dy)||0;
    ...
    for(const c of city){if(used.has(c))continue;const d=dist(c.site,tgt);if(d<bd){bd=d;best=c;}} if(!best)continue; used.add(best);
    const c=centroid(best.poly),roof=(b.affordances||[]).map(a=>LM_ROOF[a]).find(Boolean)||'#a8472b';
    ctx.fillStyle='rgba(40,28,12,.3)';ctx.fillRect(c[0]-10,c[1]-9,22,22);
    ctx.fillStyle='#e3cda0';ctx.fillRect(c[0]-11,c[1]-11,22,22);ctx.fillStyle=roof;ctx.fillRect(c[0]-11,c[1]-11,22,9);
    ...
    ctx.font='12px Georgia';ctx.textAlign='center';ctx.textBaseline='bottom';const w=ctx.measureText(b.name).width;
    ctx.fillStyle='rgba(245,236,212,.95)';ctx.strokeStyle='#5a4222';ctx.lineWidth=0.8;rr(ctx,c[0]-w/2-6,c[1]-37,w+12,17,4);ctx.fill();ctx.stroke();
    ctx.fillStyle='#2c2113';ctx.fillText(b.name,c[0],c[1]-23);
    hits.push({x:c[0],y:c[1],r:16,id:b.id,name:b.name,kind:kindFromAff(b.affordances),go:b.go,landmark:true});
  }
  // обрамление
